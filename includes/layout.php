...
function topNav(string $activePage): void {
    ...
    foreach ($pages as $key => [$label, $href]) {
        ...
        $links .= "<a class=\"{$active}\" href=\"/sanctuary/{$href}\">{$label}</a>\n";
    }
    echo <<<HTML
...
function sidebar(string $activePage): void {
    ...
    foreach ($pages as $key => [$icon, $label, $href]) {
        ...
        $links .= "<a href=\"/sanctuary/{$href}\" class=\"{$active}\"><span class=\"material-symbols-outlined\">{$icon}</span><span class=\"font-medium text-sm\">{$label}</span></a>\n";
    }
    echo <<<HTML
...
function bottomNav(string $activePage): void {
    ...
    foreach ($pages as $key => [$icon, $label, $href]) {
        if ($key === $activePage) {
            $items .= <<<HTML
<a href="/sanctuary/{$href}" class="flex flex-col items-center gap-0.5">
...
HTML;
            ...
            $items .= <<<HTML
<a href="/sanctuary/{$href}" class="flex flex-col items-center gap-0.5 text-[var(--text-muted)]">
